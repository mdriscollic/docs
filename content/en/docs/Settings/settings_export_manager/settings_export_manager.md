...
To update an interval, enter an integer in the corresponding field, then click **Save.**

Note: the deletion is based on comparing the date at midnight, so the jobs will be deleted one day later than the specified interval. 
