...
*   **Vendor invoice number.** The number provided by the vendor for an invoice. This is in contrast to the FOLIO invoice number, which is a unique FOLIO value autogenerated by the system and applied to an invoice.
...
The capabilities and capability sets listed below allow you to interact with the Invoices app and determine what you can or cannot do within the app. You can assign capabilities and capability sets to users via user roles. If none of these capabilities or capability sets are assigned to a user, they are unable to see the Invoices app or any related information.
...
|**Invoice: Export search results.** | **UI-Invoice Export CSV** | procedural | execute | This capability allows the user to export a file containing invoice information for a set of invoices.|
...
*   **FOLIO invoice number.** The autogenerated unique FOLIO identifier for the invoice. 
...
*   **Status.** The status of the invoice: Open, Reviewed, Approved, Paid, Cancelled.
*   **Total amount (system).** The combined cost of the items ordered and adjustments for the invoice. This amount will display in the currency specified on the invoice.
*   **Vendor name.** The name of the vendor from the corresponding organization record.
*   **FOLIO invoice number.** The autogenerated unique FOLIO identifier for the invoice.
...
*   **POL number.** The purchase order line number. The chain link icon facilitates POL lookup. The clipboard icon allows quick copy to your clipboard.
*   **Description.** The ordered title from the purchase order line or a description of the invoice line.
...
*   **Sub-total.** The price of the invoice before adding adjustments. This amount will display in the currency specified on the invoice.
*   **Adjustments.** The adjustments associated with the invoice. This amount will display in the currency specified on the invoice.
*   **Release encumbrance.** Displays if the corresponding checkbox on the invoice line is selected.
*   **Total.** The total amount of the invoice line calculated as the **Sub-total** plus **Adjustments**. This amount will display in the currency specified on the invoice.
*   **Total (Exchanged).** This field appears if the invoice is paid in a currency other than the system default and shows the exchanged amount in the system currency.
...
1. [Search for the invoice you want to view](#searching-for-an-invoice) and select it.
2. Click on the invoice line you wish to view
3. In the **View invoice line** pane, click the **clock icon** which is to the right of the tag icon.
...
*   **Source.**  The user who saved this version of the invoice line.
*   ***Current Version, Original Version,* or blank.**  This information displays only for the current and original versions of the invoice line.
*   **Changes.**  Displays the list of fields that were edited in this version.
4. To view the invoice line pane for a version, either click on the underlined version date mm/dd/yyyy, hh:mm or the clock icon for the version you want to open.
5. The invoice line pane displays the invoice line data for that version.  Changes from the prior version are highlighted in yellow.
6. To close the version history view, click on the X in the top left of the **Version history** pane.  The fourth pane closes and the invoice line pane displays the current invoice line version.
